...
def generate_random_maps(
    ...
    '''
    Randomly generate maps with the given length and breadth, resolution and
    obstacles.
    :param length: The length of the map in meters.
    :param breath: TODO.... The breadth of the map.
    ...
    '''
    assert breadth is None, "The current function only set's up square maps!!"
    box_length, box_width = 1.5, 1.5
    cir_radius = 0.75

    np.random.seed(seed)

    xy_circle = np.random.rand(num_circle, 2)*(length-1)+0.5

    xy_box = np.random.rand(num_box, 2)*(length-1)+0.5
    ...
    plt_boundary = plt.Rectangle((0,0), length, length, color='k', fill=None, lw=2, alpha=1,)
